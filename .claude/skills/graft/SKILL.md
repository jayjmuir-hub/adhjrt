...
## This repo (adhjrt) — two local caveats

This file is a copy of the Quins Club Hub skill; keep everything above this
section identical to that copy so drift shows in a diff.

- **The four big page files are NOT in the graph.** graft indexes JavaScript
  and Python only, and `Quins JRT.dc.html`, `Scores & Standings.dc.html`,
  `Organizer.dc.html` and `app.html` carry their code inside HTML. graft fully
  covers `netlify/functions/`, `scores-data.js`, `organizer-data.js`, `tests/`
  and `tools/`; for the page files, use the reading map in `CLAUDE.md`.
- **`graft/` is a local, git-ignored cache** — it is not committed (this repo's
  root is the deployed site) and a fresh clone will not have it. Any graft
  command rebuilds it automatically on first use; nothing extra to run.

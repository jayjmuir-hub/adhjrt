...
**Durable reference.** How this site is built, what each part does, and the
rulings that cost real effort to discover. Read the section you need; do not
read this file end to end.

⚠️ **This file is NOT for:**

- **Rules.** Git route, branch rules, secret handling, verification standards
  and reading order are in **`CLAUDE.md`**. If you are about to write a rule
  here, it goes there instead.
- **Status.** What is shipped, merged, deployed, blocked or outstanding is in
  **`claude/state-of-play.md`**, which is allowed to rot. If you are about to
  write a date, a count, a deploy id or a "currently" here — **stop, it goes
  there.**
- **History.** Why a settled question was settled lives in `claude/specs/`,
  `claude/plans/` and `claude/archive/`.

**Precedence: the code wins, then this file, then `claude/state-of-play.md`,
then `CLAUDE.md`.** Where this file and the code disagree, the code is right and
this file is a bug.
...
| `/rules` | `rules.html` — tournament rules. Same shape as `/legal`: a plain static file, not a DC component. Deliberately INDEXABLE and in `sitemap.xml`, unlike `/register-club` |
...
static photo in the About section until 5 Aug 2026, when the rotating board
replaced it. The file is left in place deliberately — it is a good shot and
costs nothing sitting there — but nothing loads it. If you are wondering why
...
| `submit-registration.js` | the public submission gateway — honeypot, rate limit, allow-list, validation, registration window, team code, sheet append, email |
| `_intake.js` | the gateway's guts: the sheet column order, the allow-list, the validation rules, the parked dead-letter blob |
| `_sheets.js` | the Google Sheets client — service-account auth, append, read |
| `_ratelimit.js` | the shared IP buckets. ⚠️ Keyed on `x-nf-client-connection-ip`, NOT `x-forwarded-for` |
...
self-signup — the function tests `!process.env.ORGANIZER_INVITE_CODE` and
fails CLOSED. Do not "fix" the missing variable. Re-adding it re-opens
self-signup, which is the documented recovery path and nothing else.
...
| `<section id="sponsors">` | **150px** | "Principal partner", plus a paragraph. 64px → 96px → **150px**, each step at Jay's request. ⚠️ **150 IS THE CEILING, NOT A TASTE.** The card's inner column caps at 560px and the lockup is 3.71:1, so 150px wants 557px of width; anything larger is clamped straight back by `max-width:100%` and renders identically — a change that appears not to have worked. ⚠️ **The rendered size lives in the CSS rule, not the tag.** The inline tag still reads `height:96px` as the no-CSS fallback and because a fault is pinned to that literal; editing it orphans the fault silently. ⚠️ **Nothing changes on a phone** — at 96px the mark was already ~356px wide against ~358px of content, so the width cap already governed. ⚠️ **All the sizes are asserted** — a placement quietly SHRINKING is the same class of failure as one quietly vanishing: the mark is still there, so nothing looks broken, and the only confirmed partner is smaller than the day before with nobody the wiser. |

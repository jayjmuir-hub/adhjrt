...
**Still open, deliberately not done here:** `accounts-admin` `password` resets any account regardless of role (JRT-19). Limiting `create` and `password` to organiser accounts — the fuller enforcement the earlier draft of this card described — was considered and **not** done, because minting a manager login IS the break-glass; taking it away defeats the purpose. Role-limiting the *reset* is JRT-19's separate call.

...
| `test-intake.js` | the sheet column order and the round trip between the writer and the two readers — 68 checks |
| `test-accounts.js` | creating manager and organiser logins, password resets, the length floor — 89 checks |
| `_prove-registration.js` | the fault injection. Not a test; a check on the tests. 87 faults, all caught by the named check |

...
fault has to be caught by name — **621 of them as of 6 Aug 2026**, and the number
...
| `_prove-registration.js` | the fault injection. Not a test; a check on the tests. 621 faults, all caught by the named check, 31 suites clean undamaged |
| `test-about-board.js` | the About-section photo ring: the encodeCase trap that broke it live, the ring geometry, the `sizes` attribute, the photo files, the three CSS traps and the two boot bugs, plus (on the Compare branch) the crest/bat pairing, the header condense loop and the two drop-down animations, and the coverflow carousel that replaced the ring — 226 checks |

...
| `test-intake.js` | the sheet column order, the round trip between the writer and the two readers, and the allow-list of what a submission may contain — 159 checks |
| `test-accounts.js` | creating manager and organiser logins, password resets, the length floor — 89 checks |
| `_prove-registration.js` | the fault injection. Not a test; a check on the tests. 98 faults, all caught by the named check |

...
const NEEDED = [
  ...
  /* Draw rights (Sep 2026). TENTH time — test-draw-rights.js REQUIRES these. */
  path.join('netlify', 'functions', '_drawRights.js'),
  path.join('netlify', 'functions', '_drawHistory.js'),
  path.join('netlify', 'functions', 'draw-history.js'),
  path.join('netlify', 'functions', 'draw-review.js'),
  /* Pitch marshals (Sep 2026) — test-pitch-marshals.js REQUIRES these, and
     reads qr.js and app.html as text. */
  path.join('netlify', 'functions', '_marshal.js'),
  path.join('netlify', 'functions', 'marshal-links.js'),
  path.join('netlify', 'functions', 'marshal-info.js'),
  path.join('netlify', 'functions', 'get-result-history.js'),
...
const FAULTS = [
  ...
  },
  {
    ...
    suite: 'test-draft-visibility.js',
    /* Repointed 8 Sep 2026: readStore takes the session too now (get-results
       strips names from the public answer). The fault is unchanged: the
       override fetch forgets the session. */
    apply: () => patch(SD,
      '  const ag = findAg(agId); if (!ag) return [];\n  const [store, state] = await Promise.all([readStore(session), fetchOverrideState(agId, session)]);',
      '  const ag = findAg(agId); if (!ag) return [];\n  const [store, state] = await Promise.all([readStore(session), fetchOverrideState(agId)]);'),
  ...
  /* ---- Draw rights (Sep 2026) ------------------------------------------
     spec-draw-rights-sep-2026.md. Driven in test-draw-rights.js. */
  {
    name: 'a manager with no switches can save the draw (results-only default gone)',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (!r.pools && !r.times) return NO_RIGHTS;', '  if (false) return NO_RIGHTS;'),
    expect: ['no switches: refused with the organisers-manage sentence'],
  },
  {
    name: 'a pools-only manager can change kickoff times (the times comparison is dropped)',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (!r.times && timesChanged(opts.stored, opts.incoming)) return POOLS_ONLY;', ''),
    expect: ['pools-only changing ONE kickoff is refused'],
  },
  {
    name: 'the times comparison ignores a pitch change',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      "    if (!same(was.startMins, sl.startMins) || !same(was.pitch || '', sl.pitch || '')) return true;",
      '    if (!same(was.startMins, sl.startMins)) return true;'),
    expect: ['pools-only changing a pitch is refused'],
  },
  {
    name: 'the times comparison is inverted, so nothing a pools-only manager sends can save',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (!r.times && timesChanged(opts.stored, opts.incoming)) return POOLS_ONLY;',
      '  if (!r.times && !timesChanged(opts.stored, opts.incoming)) return POOLS_ONLY;'),
    expect: ['pools-only changing only the team list is ALLOWED'],
  },
  {
    name: 'a times-only manager can change the team list',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (!r.pools && poolsChanged(opts.stored, opts.incoming)) return TIMES_ONLY;', ''),
    expect: ['times-only changing a team list is refused'],
  },
  {
    name: 'a partial-rights manager saves the FIRST draft against nothing',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (!opts.stored) return FIRST_DRAFT;', ''),
    expect: ['pools-only with NO stored draft: ask an organiser to save once'],
  },
  {
    name: 'the freeze is removed',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  if (now >= dayStart && isTournamentWindow(now)) {', '  if (false) {'),
    expect: ['and frozen at 00:00 on their day'],
  },
  {
    name: 'the freeze starts on day ONE for every group',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      '  const dayStart = startOfDayUTC(DEFAULT_VENUE[dayId].date);', '  const dayStart = startOfDayUTC(DEFAULT_VENUE.day1.date);'),
    expect: ['a day-2 manager is FREE on day 1'],
  },
  {
    name: 'the freeze applies to organisers too',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawRights.js'),
      "  if (!session || session.role !== 'manager') return null;\n  const dayId", '  if (!session) return null;\n  const dayId'),
    expect: ['an organiser is never frozen'],
  },
  {
    name: 'draw rights are read from the TOKEN payload instead of the stored account',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_auth.js'),
      "      drawPools: account.role === 'manager' && account.drawPools === true,\n      drawTimes: account.role === 'manager' && account.drawTimes === true,",
      '      drawPools: payload.drawPools === true,\n      drawTimes: payload.drawTimes === true,'),
    expect: ['a token that CLAIMS both rights is still refused'],
  },
  {
    name: 'managers can publish again on tournament days',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_publish.js'),
      "  if (session.role === 'manager') {\n    return 'Only tournament organisers can publish fixtures. Send your draft for review instead.';\n  }",
      "  if (session.role === 'manager') {\n    if (!isTournamentWindow(now)) return 'Only tournament organisers can publish fixtures. Send your draft for review instead.';\n    return null;\n  }"),
    expect: ['a manager on a tournament day may NOT'],
  },
  {
    name: 'get-schedule-override tells managers they can publish on the day again',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-schedule-override.js'),
      '      managerCanPublishNow: false,', "      managerCanPublishNow: require('./_publish').isTournamentWindow(),"),
    expect: ['managerCanPublishNow is false — even ON a tournament day'],
  },
  {
    name: 'get-schedule-override stops handing the caller their rights',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-schedule-override.js'),
    ...
    expect: ['hands a signed-in caller their rights'],
  },
  {
    name: 'accounts-admin lets a right be set on an organiser',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'accounts-admin.js'),
      "        if (accounts[idx].role !== 'manager') {\n          return { statusCode: 400,", "        if (false) {\n          return { statusCode: 400,"),
    expect: ['setting a right on an ORGANISER is 400'],
  },
  {
    name: 'accounts-admin stores a truthy string as a right',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'accounts-admin.js'),
      '        accounts[idx].drawPools = payload.drawPools === true;\n        accounts[idx].drawTimes = payload.drawTimes === true;',
      '        accounts[idx].drawPools = !!payload.drawPools;\n        accounts[idx].drawTimes = !!payload.drawTimes;'),
    expect: ['truthy non-booleans are NOT rights'],
  },
  {
    name: 'the Draw tab lets a manager edit whatever they like (the page gate is gone)',
    suite: 'test-draw-rights.js',
    apply: () => patch('Manager.dc.html',
      '  mayEdit(kind) {\n    const r = this.drawRights();', '  mayEdit(kind) {\n    return true;\n    const r = this.drawRights();'),
    expect: ['picking a team does nothing', 'but changing a kickoff is refused on the page'],
  },
  {
    name: 'the Draw tab derives a manager\'s rights for itself before the draft answers',
    suite: 'test-draw-rights.js',
    apply: () => patch('Manager.dc.html',
      "    return { pools: org, times: org, frozen: false, frozenNote: '' };", "    return { pools: true, times: true, frozen: false, frozenNote: '' };"),
    expect: ['a manager before the draft answers is read-only'],
  },
  {
    name: 'the account card sends only the flag just toggled, dropping the other',
    suite: 'test-draw-rights.js',
    apply: () => patch('Organizer.dc.html',
      '    const next = { drawPools: acct.drawPools === true, drawTimes: acct.drawTimes === true, ...patch };',
      '    const next = { drawPools: false, drawTimes: false, ...patch };'),
    expect: ['ticking times keeps pools on'],
  },
  /* ---- Draw rights part 2: review and history (Sep 2026) ---------------- */
  {
    name: 'a save stops filing the previous draft as history',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'save-schedule-override.js'),
      '    if (stored) {\n      try { await fileHistory(', '    if (false) {\n      try { await fileHistory('),
    expect: ['twelve overwrites keep exactly ten entries'],
  },
  {
    name: 'history keeps everything (the prune is gone)',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', '_drawHistory.js'),
      '  for (const k of keys.slice(KEEP)) {', '  for (const k of []) {'),
    expect: ['twelve overwrites keep exactly ten entries'],
  },
  {
    name: 'history files the NEW draft instead of the one being replaced',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'save-schedule-override.js'),
      'await fileHistory(store, ageGroupId, stored, { savedBy: session.username, cleared: !!reset });',
      'await fileHistory(store, ageGroupId, schedule || stored, { savedBy: session.username, cleared: !!reset });'),
    expect: ['the newest entry is the draft that was REPLACED'],
  },
  {
    name: 'a restore overwrites the draft without filing it first (not undoable)',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-history.js'),
      '      if (current) await fileHistory(store, ageGroupId, current, { savedBy: session.username });', ''),
    expect: ['restore filed the previous current draft FIRST'],
  },
  {
    name: 'a manager can read and restore draw history',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-history.js'),
      "    if (session.role !== 'organizer') return json(403,", '    if (false) return json(403,'),
    expect: ['a manager cannot read the history'],
  },
  {
    name: 'send-for-review loses its rate limit',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-review.js'),
      '    if (!rate.ok) return tooManyResponse(rate);', ''),
    expect: ['a second request inside ten minutes is 429'],
  },
  {
    name: 'send-for-review emails unapproved organisers too',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-review.js'),
      "    .filter((a) => a.role === 'organizer' && a.approved && typeof a.email === 'string' && a.email.includes('@'))",
      "    .filter((a) => a.role === 'organizer' && typeof a.email === 'string' && a.email.includes('@'))"),
    expect: ['to every APPROVED organiser with an address'],
  },
  {
    name: 'a manager with no edit right can send for review',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-review.js'),
      "    if (session.role === 'manager' && !r.pools && !r.times) {", '    if (false) {'),
    expect: ['a manager with NO edit right cannot send for review'],
  },
  {
    name: 'publishing leaves the review note behind',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'publish-schedule.js'),
      "    try { await store.delete(`review:${ageGroupId}`); } catch (e) { /* see above */ }", ''),
    expect: ['clears the review record'],
  },
  {
    name: 'a mail outage fails the review request',
    suite: 'test-draw-rights.js',
    apply: () => patch(path.join('netlify', 'functions', 'draw-review.js'),
      "      console.warn('draw-review: email failed -', err && err.message);", "      return json(500, { ok: false, error: 'mail' });"),
    expect: ['with the mail service down the request still succeeds'],
  },
  {
    name: 'the Draw tab offers Send for review with unsaved changes',
    suite: 'test-draw-rights.js',
    apply: () => patch('Manager.dc.html',
      "(dr.pools || dr.times) && !dr.frozen && d && !s.drawDirty),", "(dr.pools || dr.times) && !dr.frozen && d),"),
    expect: ['but not with unsaved changes'],
  },
  /* ---- Pitch marshals (Sep 2026) ---------------------------------------
     spec-pitch-marshals-sep-2026.md. Driven in test-pitch-marshals.js with
     the clock pinned to DEFAULT_VENUE. */
  {
    name: 'a marshal link works on any day',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', '_marshal.js'),
      "  if (!isLiveDay(day, now)) return { ok: false, reason: 'not-today' };", ''),
    expect: ['not the night before'],
  },
  {
    name: 'a reissued link keeps working (the jti is not compared)',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', '_marshal.js'),
      "  if (!record || record.jti !== jti) return { ok: false, reason: 'reissued' };", "  if (!record) return { ok: false, reason: 'reissued' };"),
    expect: ['Issue new kills the old link'],
  },
  {
    name: 'a revoked link keeps working',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', '_marshal.js'),
      "  if (record.revokedAt) return { ok: false, reason: 'revoked' };", ''),
    expect: ['and the link is dead'],
  },
  {
    name: 'the listing carries the token',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', '_marshal.js'),
      ...
      "  const token = sign({ kind: 'marshal', ageGroupId, day, pitch, jti });\n  const record = { jti, token, issuedAt: new Date().toISOString(), issuedBy, revokedAt: null };\n  await marshalStore().setJSON(recordKey(ageGroupId, day, pitch), record);"),
    expect: ['nor does the store'],
  },
  {
    name: 'another group\'s manager can issue links for this group',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'marshal-links.js'),
      "    if (!hasAgeGroupAccess(session, ageGroupId)) return json(403,", '    if (false) return json(403,'),
    expect: ["another group's manager cannot issue for this group"],
  },
  {
    name: 'submit-result stops checking the marshal\'s pitch',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
      "      if ((slot.pitch || '') !== marshal.pitch) {", '      if (false) {'),
    expect: ['a Pitch B link cannot save a Pitch A match'],
  },
  {
    name: 'submit-result stops checking the marshal\'s age group',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
      '      if (marshal.ageGroupId !== agId) {', '      if (false) {'),
    expect: ["nor another group's match"],
  },
  {
    name: 'a marshal can clear a result',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
      "      if (data.clear === true) {\n        return { statusCode: 403, body: JSON.stringify({ ok: false, error: 'Clearing a result is done at the age-group table.' }) };\n      }", ''),
    expect: ['a marshal cannot clear'],
  },
  {
    name: 'a marshal saves with no name',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
  ...
  {
    name: 'a marshal overwrites the table\'s score (precedence gone)',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
      "      if (existing && kind !== 'marshal') {", '      if (false) {'),
    expect: ["a marshal cannot overwrite the TABLE's score"],
  },
  {
    name: 'overwriting a result stops filing the previous entry',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'submit-result.js'),
      '      if (previous) await fileResultHistory(store, matchId, previous);', ''),
    expect: ['and the previous entry is in the history'],
  },
  {
    name: 'result history keeps everything',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', '_results.js'),
      '  await store.setJSON(historyKey(matchId), list.slice(0, HISTORY_KEEP));', '  await store.setJSON(historyKey(matchId), list);'),
    expect: ['history keeps exactly twenty'],
  },
  {
    name: 'get-results serves the record whole to the public again',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-results.js'),
      "    if (trusted) return { statusCode: 200, body: JSON.stringify({ ok: true, results }) };",
      "    return { statusCode: 200, body: JSON.stringify({ ok: true, results }) };"),
    expect: ['but no username'],
  },
  {
    name: 'get-results puts the Spirit nominees on the public allow-list',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-results.js'),
      "  'homeDrops', 'awayDrops', 'homeCards', 'awayCards', 'walkover', 'submittedAt',",
      "  'homeDrops', 'awayDrops', 'homeCards', 'awayCards', 'walkover', 'submittedAt', 'spiritNomineeHome', 'spiritNomineeAway',"),
    expect: ['no Spirit nominees'],
  },
  {
    name: 'get-results trusts a marshal token as a session',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-results.js'),
      "    const trusted = !!(session && (session.role === 'manager' || session.role === 'organizer'));",
      "    const trusted = !!(session && (session.role === 'manager' || session.role === 'organizer')) || !!require('./_auth').getBearerToken(event);"),
    expect: ['a marshal token gets the PUBLIC shape'],
  },
  {
    name: 'get-result-history answers any manager',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'get-result-history.js'),
      "    if (!hasAgeGroupAccess(session, agId)) return json(403,", '    if (false) return json(403,'),
    expect: ["another group's manager cannot"],
  },
  {
    name: 'marshal-info answers a session token',
    suite: 'test-pitch-marshals.js',
    apply: () => patch(path.join('netlify', 'functions', 'marshal-info.js'),
      '    if (!v.ok) return json(401, { ok: false, error: NOT_LIVE });',
      "    if (!v.ok && v.reason !== 'not-marshal') return json(401, { ok: false, error: NOT_LIVE });\n    if (!v.ok) v.marshal = { ageGroupId: 'u16b', day: 'day2', pitch: 'D2' };"),
    expect: ['a session token is refused here'],
  },
  {
    name: 'the app clears its timer interval',
    suite: 'test-pitch-marshals.js',
    apply: () => patch('app.html',

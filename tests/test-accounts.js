...

{
  ...
  /* Every place a password is SET enforces it. manager-signup.js was in this
     list until it was deleted with manager self-signup (JRT-30, 12 Sep 2026);
     two password-setting endpoints remain — organiser self-signup (the dormant
     recovery path) and the back-office create. The count is written in so
     dropping one silently shrinks the sweep. */
  const passwordEndpoints = ['organizer-signup.js', 'accounts-admin.js'];
  eq('two password-setting endpoints are swept', passwordEndpoints.length, 2);
  passwordEndpoints.forEach((f) => {
...
/* ------------------------------------------------------------------------
   THE ALL-GROUPS SENTINEL IN _auth.js (added 5 Aug 2026; trimmed 12 Sep 2026)

   ⚠️ This section once proved that the master MANAGER CODE and the all-groups
   auth test agreed: manager-signup.js stored whichever KEY NAME in
   MANAGER_INVITE_CODES matched the code as the account's ageGroupId, and
   `_auth.js` grants every group when `session.ageGroupId === '*'`. Both setup
   instructions once said to name the master key "admin", which minted a
   manager scoped to a group that does not exist — a bug that failed CLOSED and
   so never announced itself.

   Manager self-signup — and with it the master-code mechanism and its setup
   comment — was retired on 12 Sep 2026 (JRT-30); an all-access person is an
   organiser now. What still needs guarding is the sentinel itself: the `*`
   test in `_auth.js` governs any restored legacy all-groups account, so
   changing it must not pass unnoticed. See the break-glass decision card. */
section('the all-groups sentinel in _auth.js');
